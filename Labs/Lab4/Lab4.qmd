...
```{r sumario modelo fixo}
#| label: tbl-sumario-fixo
summary(modelo_fixo) |> pander::pander(caption = "Sumário do modelo de efeitos fixos completo.")
```

Nota-se na @tbl-sumario-fixo um baixíssimo valor de R² ajustado, indicando que o
modelo não explica bem a variabilidade dos dados. Além disso, vários
coeficientes não são significativos. Faremos uma análise diagnóstica do modelo
para verificar se as suposições do modelo linear clássico são válidas.
...
Pelos gráficos dos resíduos na @fig-residuos-fixo, os resíduos possuem
autocorrelação não nula nas vacas e apresentam padrões na análise dos resíduos
padronizados. ```

Verificamos a normalidade pelo QQ-plot em
@fig-qq-fixo.
...
E numericamente com o teste de Shapiro-Wilk na @tbl-shapiro-fixo.
...
Não rejeitada a normalidade, verificaremos a homocedasticidade com o teste de Bartlett na
@tbl-bartlett-fixo.
...
Conferimos o sumário do modelo final na @tbl-sumario-final.

```{r sumario modelo final}
#| label: tbl-sumario-final
summary(mod_final) |> pander::pander(caption = "Sumário do modelo misto.")
```

Pelo critério AIC, com valor de $163.76$, o modelo de efetios mistos final é
bastante superior ao modelo de efeitos fixos, com AIC de $577.70$.
