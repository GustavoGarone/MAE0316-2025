...
#| fig-cap: "Boxplot dos resíduos do modelo com efeitos fixos por vaca."
...
Pelo gráfico da @fig-residuos-fixo, os resíduos possuem autocorrelação não nula
nas vacas. Verificaremos a homocedasticidade com os testes de
Bartlett em @tbl-bartlett-fixo.
...
Partiremos para a análise diagnóstica desse modelo.
...
Pelo gráfico da @fig-residuos-mistos, os resíduos parecem mais bem comportados
em relação ao modelo com efeitos fixos. Verificaremos a homocedasticidade com o
teste de Bartlett em @tbl-bartlett-mistos.

```{r Teste Bartlett Mistos}
#| label: tbl-bartlett-mistos
#| tbl-cap: "Teste de Bartlett para homocedasticidade dos resíduos do modelo com efeitos mistos."
teste_bm <- bartlett.test(residuos_mistos ~ dados$Cow)
tibble(
  Estatística = teste_bm$statistic,
  `Valor-p` = teste_bm$p.value
) |>
  kable(digits = 4)
```
...
E o teste de Shapiro-Wilk em @tbl-shapiro-mistos.
```{r Teste Shapiro Mistos}
#| label: tbl-shapiro-mistos
#| tbl-cap: "Teste de Shapiro-Wilk para normalidade dos resíduos do modelo com efeitos mistos."
teste_sm <- shapiro.test(residuos_mistos)
tibble(
  Estatística = teste_sm$statistic,
  `Valor-p` = teste_sm$p.value
) |>
  kable(digits = 4)
```

Com normalidade não rejeitada, realizamos o teste de levene em
@tbl-levene-mistos.
```{r Teste Levene Mistos}
#| label: tbl-levene-mistos
#| tbl-cap: "Teste de Levene para homocedasticidade dos resíduos do modelo com efeitos mistos."
teste_lm <- leveneTest(residuos_mistos ~ dados$Cow)
tibble(
  Estatística = teste_lm$`F value`[1],
  `Valor-p` = teste_lm$`Pr(>F)`[1]
) |>
  kable(digits = 4)
```

Ainda precisamos verificar a autocorrelação dos resíduos. Faremos isso com o
gráfico de autocorrelação em @fig-acf-mistos.
```{r ACF Mistos}
...
Claramente, há autocorrelação significativa nos resíduos, mas confirmaremos com
o teste de Durbin-Watson em @tbl-durbin-misto.
