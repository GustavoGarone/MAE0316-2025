...
modelo_permutacao <- lmp(Retencao ~ Grupo, data = df_grupos)
...
inválida. Prosseguiremos mesmo assim com a análise post-hoc.
...
Corroborando resultados anteriores, o teste de Dunn não aponta diferenças
significativas entre os grupos.

## Conclusões da modelagem sobre a retenção

A partir dos modelos ajustados, não conseguimos identificar diferenças
significativas na retenção de informação entre os diferentes tratamentos com os
métodos utilizados. Isso ocorre devido à forma discreta da variável resposta e a
pequena amostra, que dificultam o ajuste de modelos adequados.

Nesse contexto, seguiriamos com a análise através de transformações, como a
tranformação `Logit`, ou o uso de modelos lineares generalizados, como o modelo
logístico para uma outra forma da variável retenção, ou até mesmo modelos
binomiais.
