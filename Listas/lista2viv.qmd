...
contínua do tempo das obervações, tranformadas das datas das visitas.
...
indiretamente como as obersavações de um mesmo indivíduo ao longo do tempo (ver
...
dados longitudinais, inicaremos por um modelo completo da forma:
...
indicador binário (dummy) que vale 1 se a participante $i$ foi suplementada com
cálcio e 0 caso contrário, $A_{ij}G_{i}$ é a interação entre idade e grupo,
...
Dessa forma, podemos modear o efeito aleatório de intercepto e inclinação, bem
...
O modelo com correção ajustado não covergiu com o método `varPower`, por isso,
...
resíduos distribuídos aleatoria e uniformemente em torno de zero, o que também
reforça a independência obtida pela inclusão dos efeitos aleatórios.


Confirmaremos a homocedasticiadade com o teste de Bartlett, com resultado na
@tbl-bartlett.
...
cálcio e 0 caso contrário, $A_{ij}G_{i}$ é a interação entre idade e grupo,
$b_{0i}$ é o efeito aleatório de intercepto da participante $i$, $b_{1i}$ é o
efeito aleatório de inclinação (slope, coeficiente angular) da participante $i$
e $\epsilon_{ij}$ é o erro aleatório da observação $ij$. Ademais,
